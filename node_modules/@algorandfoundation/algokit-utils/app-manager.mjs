...
var AppManager = class AppManager {
	...
	*/
	async compileTeal(tealCode) {
		...
		const result = {
			...
			sourceMap: new ProgramSourceMap(compiled.sourcemap)
		};
	...
	*/
	...
	static hasAbiReturnPrefix(log) {
		if (log.length < ABI_RETURN_PREFIX.length) return false;
		for (let i = 0; i < ABI_RETURN_PREFIX.length; i++) if (log[i] !== ABI_RETURN_PREFIX[i]) return false;
		return true;
	}
	/**
	...
	*/
	static replaceTealTemplateDeployTimeControlParams(tealTemplateCode, params) {
		if (params.updatable !== void 0) {
			if (!tealTemplateCode.includes(UPDATABLE_TEMPLATE_NAME)) throw new Error(`Deploy-time updatability control requested for app deployment, but ${UPDATABLE_TEMPLATE_NAME} not present in TEAL code`);
		...
		if (params.deletable !== void 0) {
			if (!tealTemplateCode.includes(DELETABLE_TEMPLATE_NAME)) throw new Error(`Deploy-time deletability control requested for app deployment, but ${DELETABLE_TEMPLATE_NAME} not present in TEAL code`);
