import { ADDRESS_LENGTH, PUBLIC_KEY_BYTE_LENGTH } from "../../constants.mjs";
...
var AddressCodec = class extends Codec {
	...
	fromEncoded(value, _format) {
		...
		if (value instanceof Uint8Array) {
			if (value.length === PUBLIC_KEY_BYTE_LENGTH) return new Address(value);
			else if (value.length === ADDRESS_LENGTH) return Address.fromString(Buffer.from(value).toString("utf-8"));
		}
