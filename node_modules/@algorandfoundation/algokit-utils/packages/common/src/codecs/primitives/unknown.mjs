import { ADDRESS_LENGTH, PUBLIC_KEY_BYTE_LENGTH, SIGNATURE_BYTE_LENGTH } from "../../constants.mjs";
import { Codec } from "../codec.mjs";
import { bigIntCodec } from "./bigint.mjs";
import { booleanCodec } from "./boolean.mjs";
import { numberCodec } from "./number.mjs";
import { stringCodec } from "./string.mjs";
import { ArrayCodec } from "../composite/array.mjs";
import { RecordCodec } from "../composite/record.mjs";
...
var UnknownCodec = class extends Codec {
	...
	maybeDecodeAsUtf8(bytes) {
		try {
			if ((bytes.length === ADDRESS_LENGTH || bytes.length === PUBLIC_KEY_BYTE_LENGTH || bytes.length === SIGNATURE_BYTE_LENGTH) && bytes.every((byte) => byte === 0)) return bytes;
