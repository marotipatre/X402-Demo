...
const require_wire = require('../wire.js');

//#region packages/common/src/codecs/models/object-model.ts
...
var ObjectModelCodec = class extends require_codec.Codec {
	...
	fromEncoded(value, format) {
		const metadata = this.getMetadata();
		const normalizedWireObject = require_wire.normalizeWireObject(value);
		const result = {};
		for (const field of metadata.fields) if (field.flattened) {
			...
			const wireValue = normalizedWireObject[field.wireKey || field.name];
	...
	encodeFlattenedField(field, fieldValue, format) {
		const encoded = field.codec.encodeOptional(fieldValue, format);
		if (encoded !== void 0 && typeof encoded === "object" && !Array.isArray(encoded)) return require_wire.normalizeWireObject(encoded);
		return {};
	}
...
//#endregion
exports.ObjectModelCodec = ObjectModelCodec;
