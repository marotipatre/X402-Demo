...
function partialSignWithMultisigSignature(transaction, metadata, signerAddr, signature) {
	if (signature.length != require_constants.SIGNATURE_BYTE_LENGTH) throw new Error(MULTISIG_SIGNATURE_LENGTH_ERROR_MSG);
	return createMultisigTransactionWithSignature(transaction, {
		rawSig: signature,
		myPk: (typeof signerAddr === "string" ? require_address.Address.fromString(signerAddr) : signerAddr).publicKey
	}, metadata);
}
/**
...
function addressFromMultisigPreImg({ version, threshold, publicKeys }) {
	...
	return new require_address.Address(Uint8Array.from(require_hash.hash(merged)));
}
/**
