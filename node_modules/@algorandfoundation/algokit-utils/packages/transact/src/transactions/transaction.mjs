...
import { MAX_TRANSACTION_GROUP_SIZE, SIGNATURE_ENCODING_INCR, TRANSACTION_DOMAIN_SEPARATOR, TRANSACTION_GROUP_DOMAIN_SEPARATOR, TRANSACTION_ID_LENGTH } from "../../../common/src/constants.mjs";
...
import { getValidationErrorMessage } from "./common.mjs";
import { validateAppCallTransaction } from "./app-call.mjs";
import { validateAssetConfigTransaction } from "./asset-config.mjs";
import { validateAssetFreezeTransaction } from "./asset-freeze.mjs";
import { validateAssetTransferTransaction } from "./asset-transfer.mjs";
import { validateKeyRegistrationTransaction } from "./key-registration.mjs";
import { TransactionType } from "./transaction-type.mjs";
import { transactionParamsCodec } from "./transaction-meta.mjs";
...
function encodeTransaction(transaction) {
	...
	return concatArrays(new TextEncoder().encode(TRANSACTION_DOMAIN_SEPARATOR), rawBytes);
}
/**
...
function computeGroup(transactions) {
	...
	if (transactions.length > MAX_TRANSACTION_GROUP_SIZE) throw new Error(`Transaction group size exceeds the max limit of ${MAX_TRANSACTION_GROUP_SIZE}`);
	const txHashes = transactions.map((tx) => {
		if (tx.group) throw new Error("Transactions must not already be grouped");
		return hash(encodeTransaction(tx));
	});
	return hash(concatArrays(new TextEncoder().encode(TRANSACTION_GROUP_DOMAIN_SEPARATOR), encodeMsgpack({ txlist: txHashes })));
}
...
//#endregion
export { Transaction, assignFee, calculateFee, decodeTransaction, decodeTransactions, encodeTransaction, encodeTransactionRaw, encodeTransactions, estimateTransactionSize, getEncodedTransactionType, groupTransactions, transactionCodec, validateTransaction };
