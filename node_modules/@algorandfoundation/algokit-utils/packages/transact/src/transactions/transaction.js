...
function encodeTransaction(transaction) {
	...
	return require_array.concatArrays(new TextEncoder().encode(require_constants.TRANSACTION_DOMAIN_SEPARATOR), rawBytes);
}
/**
...
function decodeTransaction(encoded_transaction) {
	...
	const prefixBytes = new TextEncoder().encode(require_constants.TRANSACTION_DOMAIN_SEPARATOR);
	let hasPrefix = true;
...
function estimateTransactionSize(transaction) {
	...
	return BigInt(encoded.length + require_constants.SIGNATURE_ENCODING_INCR);
}
/**
...
function computeGroup(transactions) {
	...
	if (transactions.length > require_constants.MAX_TRANSACTION_GROUP_SIZE) throw new Error(`Transaction group size exceeds the max limit of ${require_constants.MAX_TRANSACTION_GROUP_SIZE}`);
	const txHashes = transactions.map((tx) => {
		if (tx.group) throw new Error("Transactions must not already be grouped");
		return require_hash.hash(encodeTransaction(tx));
	});
	return require_hash.hash(require_array.concatArrays(new TextEncoder().encode(require_constants.TRANSACTION_GROUP_DOMAIN_SEPARATOR), require_msgpack.encodeMsgpack({ txlist: txHashes })));
}
