...
//#region packages/abi/src/bigint.ts
/**
...
function bigIntToBytes(value, size) {
	...
	if (hex.length !== size * 2) hex = hex.padStart(size * 2, "0");
	const byteArray = new Uint8Array(hex.length / 2);
	for (let i = 0, j = 0; i < hex.length / 2; i++, j += 2) byteArray[i] = parseInt(hex.slice(j, j + 2), 16);
	return byteArray;
}
/**
...
function bytesToBigInt(bytes) {
	...
	for (let i = 0; i < bytes.length; i++) res = BigInt(Number(buf.getUint8(i))) + res * BigInt(256);
	return res;
}
