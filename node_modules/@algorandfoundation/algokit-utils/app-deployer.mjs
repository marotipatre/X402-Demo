...
var AppDeployer = class {
	...
	*/
	async deploy(deployment) {
		...
		if (existingAppRecord.localInts < (createParams.schema?.localInts ?? 0) || existingAppRecord.globalInts < (createParams.schema?.globalInts ?? 0) || existingAppRecord.localByteSlices < (createParams.schema?.localByteSlices ?? 0) || existingAppRecord.globalByteSlices < (createParams.schema?.globalByteSlices ?? 0) || extraPages < newExtraPages) {
			...
			if (onSchemaBreak === void 0 || onSchemaBreak === "fail" || onSchemaBreak === OnSchemaBreak.Fail) throw new Error("Schema break detected and onSchemaBreak=OnSchemaBreak.Fail, stopping deployment. If you want to try deleting and recreating the app then re-run with onSchemaBreak=OnSchemaBreak.ReplaceApp");
	...
	*/
	async getCreatorAppsByName(creator, ignoreCache) {
		...
		const createdApps = (await lookupAccountCreatedApplicationByAddress(this._indexer, creatorString)).map((a) => {
			return {
				id: a.id,
		...
		(await Promise.all(createdApps.map(async (createdApp) => {
			...
			const parseNote = (note) => {
				if (!note) return;
				if (!note.startsWith(`${APP_DEPLOY_NOTE_DAPP}:j{`)) return;
