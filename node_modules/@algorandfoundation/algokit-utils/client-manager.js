...
const require_client$1 = require('./packages/indexer_client/src/client.js');
const require_client$2 = require('./packages/kmd_client/src/client.js');
...
var ClientManager = class ClientManager {
	...
	*/
	static getAlgodClient(config) {
		...
		return new require_client.AlgodClient({
			baseUrl: server,
			port,
			token,
			logger: require_config.Config.logger
		});
	}
	/**
	...
	*/
	static getIndexerClient(config) {
		const { token, server, port } = config;
		return new require_client$1.IndexerClient({
			baseUrl: server,
			port,
			token,
			logger: require_config.Config.logger
	...
	*/
	static getKmdClient(config) {
		const { token, server, port } = config;
		return new require_client$2.KmdClient({
			baseUrl: server,
			port,
			token,
			logger: require_config.Config.logger
