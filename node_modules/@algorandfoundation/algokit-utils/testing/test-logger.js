...
//#region src/testing/test-logger.ts
...
var TestLogger = class {
	...
	*/
	getLogSnapshot(config) {
		...
		accounts?.forEach((sender, id) => snapshot = snapshot.replace(new RegExp(typeof sender === "string" ? sender : "addr" in sender ? sender.addr.toString() : sender.toString(), "g"), `ACCOUNT_${id + 1}`));
	...
	error(message, ...optionalParams) {
		...
		this.logs.push(`ERROR: ${message}${optionalParams.length ? ` | ${require_util.asJson(optionalParams)}` : ""}`);
	}
	warn(message, ...optionalParams) {
		this.originalLogger?.warn(message, ...optionalParams);
		this.logs.push(`WARN: ${message}${optionalParams.length ? ` | ${require_util.asJson(optionalParams)}` : ""}`);
	}
	info(message, ...optionalParams) {
		this.originalLogger?.info(message, ...optionalParams);
		this.logs.push(`INFO: ${message}${optionalParams.length ? ` | ${require_util.asJson(optionalParams)}` : ""}`);
	}
	verbose(message, ...optionalParams) {
		this.originalLogger?.verbose(message, ...optionalParams);
		this.logs.push(`VERBOSE: ${message}${optionalParams.length ? ` | ${require_util.asJson(optionalParams)}` : ""}`);
	}
	debug(message, ...optionalParams) {
		this.originalLogger?.debug(message, ...optionalParams);
		this.logs.push(`DEBUG: ${message}${optionalParams.length ? ` | ${require_util.asJson(optionalParams)}` : ""}`);
	}
};
