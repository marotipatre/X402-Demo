...
var AppDeployer = class {
	...
	*/
	async deploy(deployment) {
		...
		createParams.note = updateParams.note = require_composer.TransactionComposer.arc2Note({
		...
		const newExtraPages = require_util.calculateExtraProgramPages(newApprovalBytes, newClearBytes);
		...
		if (existingAppRecord.localInts < (createParams.schema?.localInts ?? 0) || existingAppRecord.globalInts < (createParams.schema?.globalInts ?? 0) || existingAppRecord.localByteSlices < (createParams.schema?.localByteSlices ?? 0) || existingAppRecord.globalByteSlices < (createParams.schema?.globalByteSlices ?? 0) || extraPages < newExtraPages) {
			require_config.Config.getLogger(sendParams?.suppressLog).warn(`Detected a breaking app schema change in app ${existingApp.appId}:`, {
				from: {
			...
			if (onSchemaBreak === void 0 || onSchemaBreak === "fail" || onSchemaBreak === require_app.OnSchemaBreak.Fail) throw new Error("Schema break detected and onSchemaBreak=OnSchemaBreak.Fail, stopping deployment. If you want to try deleting and recreating the app then re-run with onSchemaBreak=OnSchemaBreak.ReplaceApp");
			if (onSchemaBreak === "append" || onSchemaBreak === require_app.OnSchemaBreak.AppendApp) {
				require_config.Config.getLogger(sendParams?.suppressLog).info("onSchemaBreak=AppendApp, will attempt to create a new app");
				return await createApp();
			}
			if (existingApp.deletable) require_config.Config.getLogger(sendParams?.suppressLog).info("App is deletable and onSchemaBreak=ReplaceApp, will attempt to create new app and delete old app");
			else require_config.Config.getLogger(sendParams?.suppressLog).info("App is not deletable but onSchemaBreak=ReplaceApp, will attempt to delete app, delete will most likely fail");
		...
		if (isUpdate) {
			...
			if (onUpdate === "update" || onUpdate === require_app.OnUpdate.UpdateApp) {
				if (existingApp.updatable) require_config.Config.getLogger(sendParams?.suppressLog).info(`App is updatable and onUpdate=UpdateApp, updating app...`);
				else require_config.Config.getLogger(sendParams?.suppressLog).warn(`App is not updatable but onUpdate=UpdateApp, will attempt to update app, update will most likely fail`);
				return await updateApp(existingApp);
			}
			if (onUpdate === "replace" || onUpdate === require_app.OnUpdate.ReplaceApp) {
				if (existingApp.deletable) require_config.Config.getLogger(sendParams?.suppressLog).warn("App is deletable and onUpdate=ReplaceApp, creating new app and deleting old app...");
				else require_config.Config.getLogger(sendParams?.suppressLog).warn("App is not deletable and onUpdate=ReplaceApp, will attempt to create new app and delete old app, delete will most likely fail");
				return await replaceApp(existingApp);
			}
		}
		require_config.Config.getLogger(sendParams?.suppressLog).debug("No detected changes in app, nothing to do.");
		return {
			...existingApp,
	...
	updateAppLookup(sender, appMetadata) {
		const s = require_address.getAddress(sender).toString();
		const lookup = this._appLookups.get(s);
		if (!lookup) this._appLookups.set(s, {
			creator: require_address.Address.fromString(s),
	...
	*/
	async getCreatorAppsByName(creator, ignoreCache) {
		const appLookup = {};
		const creatorAddress = require_address.getAddress(creator);
		const creatorString = creatorAddress.toString();
		if (!ignoreCache && this._appLookups.has(creatorString)) return this._appLookups.get(creatorString);
		...
		const createdApps = (await require_indexer_lookup.lookupAccountCreatedApplicationByAddress(this._indexer, creatorString)).map((a) => {
			return {
				id: a.id,
		...
		(await Promise.all(createdApps.map(async (createdApp) => {
			const appTransactions = await require_indexer_lookup.searchTransactions(this._indexer, {
				minRound: createdApp.createdAtRound,
				txType: require_transaction_type.TransactionType.AppCall,
				applicationId: createdApp.id,
				address: creatorAddress,
				addressRole: "sender",
				notePrefix: Buffer.from(require_app.APP_DEPLOY_NOTE_DAPP).toString("base64")
			});
			...
			const parseNote = (note) => {
				if (!note) return;
				if (!note.startsWith(`${require_app.APP_DEPLOY_NOTE_DAPP}:j{`)) return;
				return JSON.parse(note.substring(require_app.APP_DEPLOY_NOTE_DAPP.length + 2));
			};
			try {
				...
				if (creationNote?.name) appLookup[creationNote.name] = {
					appId: createdApp.id,
					appAddress: require_address.getApplicationAddress(createdApp.id),
				...
				require_config.Config.logger.warn(`Received error trying to retrieve app with ${createdApp.id} for creator ${creatorAddress}; failing silently`, e);
				return;
			}
		});
		const lookup = {
