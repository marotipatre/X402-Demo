...
function populateTransactionResources(transaction, resourcesAccessed, groupIndex) {
	...
	if (accountsCount + assetsCount + appsCount + boxesCount > MAX_OVERALL_REFERENCES) throw new Error(`Resource reference limit of ${MAX_OVERALL_REFERENCES} exceeded in transaction ${groupIndex}`);
}
...
function populateGroupResource(transactions, resource) {
	if (resource.type === GroupResourceType.AssetHolding || resource.type === GroupResourceType.AppLocal) {
		...
		const groupIndex2 = transactions.findIndex((txn) => {
			if (!isAppCallBelowResourceLimit(txn)) return false;
			const appCall = txn.appCall;
			if ((appCall.accountReferences?.length ?? 0) >= MAX_ACCOUNT_REFERENCES) return false;
	...
	const groupIndex = transactions.findIndex((txn) => {
		...
		switch (resource.type) {
			case GroupResourceType.Account: return accountsCount + assetsCount + appsCount + boxesCount < MAX_OVERALL_REFERENCES;
			case GroupResourceType.AssetHolding:
			case GroupResourceType.AppLocal: return accountsCount + assetsCount + appsCount + boxesCount < MAX_OVERALL_REFERENCES - 1;
			case GroupResourceType.Box: if (resource.data.appId !== 0n) return accountsCount + assetsCount + appsCount + boxesCount < MAX_OVERALL_REFERENCES - 1;
			else return accountsCount + assetsCount + appsCount + boxesCount < MAX_OVERALL_REFERENCES;
