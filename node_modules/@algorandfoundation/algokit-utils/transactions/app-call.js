...
const buildAppCreate = async (params, appManager, suggestedParams, defaultValidityWindow) => {
	...
	return new require_transaction.Transaction({
		...commonData,
		type: require_transaction_type.TransactionType.AppCall,
		appCall: {
			appId: 0n,
...
function populateTransactionResources(transaction, resourcesAccessed, groupIndex) {
	if (transaction.type !== require_transaction_type.TransactionType.AppCall || transaction.appCall === void 0) return;
	...
	if (accountsCount + assetsCount + appsCount + boxesCount > require_constants.MAX_OVERALL_REFERENCES) throw new Error(`Resource reference limit of ${require_constants.MAX_OVERALL_REFERENCES} exceeded in transaction ${groupIndex}`);
}
...
function isAppCallBelowResourceLimit(txn) {
	if (txn.type !== require_transaction_type.TransactionType.AppCall) return false;
	...
	return accountsCount + assetsCount + appsCount + boxesCount < require_constants.MAX_OVERALL_REFERENCES;
}
/**
...
function populateGroupResource(transactions, resource) {
	if (resource.type === GroupResourceType.AssetHolding || resource.type === GroupResourceType.AppLocal) {
		...
		const groupIndex1 = transactions.findIndex((txn) => {
			...
			if (appCall.appReferences) {
				for (const appId of appCall.appReferences) if (address === require_address.getApplicationAddress(appId)) return true;
			}
		...
		const groupIndex2 = transactions.findIndex((txn) => {
			if (!isAppCallBelowResourceLimit(txn)) return false;
			const appCall = txn.appCall;
			if ((appCall.accountReferences?.length ?? 0) >= require_constants.MAX_ACCOUNT_REFERENCES) return false;
	...
	const groupIndex = transactions.findIndex((txn) => {
		...
		switch (resource.type) {
			case GroupResourceType.Account: return accountsCount + assetsCount + appsCount + boxesCount < require_constants.MAX_OVERALL_REFERENCES;
			case GroupResourceType.AssetHolding:
			case GroupResourceType.AppLocal: return accountsCount + assetsCount + appsCount + boxesCount < require_constants.MAX_OVERALL_REFERENCES - 1;
			case GroupResourceType.Box: if (resource.data.appId !== 0n) return accountsCount + assetsCount + appsCount + boxesCount < require_constants.MAX_OVERALL_REFERENCES - 1;
			else return accountsCount + assetsCount + appsCount + boxesCount < require_constants.MAX_OVERALL_REFERENCES;
			default: return accountsCount + assetsCount + appsCount + boxesCount < require_constants.MAX_OVERALL_REFERENCES;
		}
	});
...
//#endregion
exports.buildAppCall = buildAppCall;
exports.buildAppCreate = buildAppCreate;
exports.buildAppUpdate = buildAppUpdate;
exports.populateGroupResources = populateGroupResources;
exports.populateTransactionResources = populateTransactionResources;
//# sourceMappingURL=app-call.js.map