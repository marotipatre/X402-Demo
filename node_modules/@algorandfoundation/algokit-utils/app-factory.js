...
var AppFactory = class {
	...
	*/
	...
	send = {
		...
		create: async (params) => {
			...
			const result = await this.handleCallErrors(async () => {
				const result = await this._algorand.send.appCreateMethodCall(await this.params.create({
					...params,
					updatable,
					deletable,
					deployTimeParams
	...
	*/
	async deploy(params) {
		...
		return {
			...
			result: {
				...result,
				return: "return" in result ? result.operationPerformed === "update" ? params.updateParams && "method" in params.updateParams ? result.return?.returnValue : void 0 : params.createParams && "method" in params.createParams ? result.return?.returnValue : void 0 : void 0,
				deleteReturn: "deleteReturn" in result && params.deleteParams && "method" in params.deleteParams ? result.deleteReturn?.returnValue : void 0
			}
		};
	}
	/**
	...
	*/
	...
	getDeployTimeControl(control) {
		...
		if (!approval || !approval.includes(control === "updatable" ? require_app.UPDATABLE_TEMPLATE_NAME : require_app.DELETABLE_TEMPLATE_NAME)) return void 0;
	...
	getParamsMethods() {
		return {
			create: async (params) => {
				...
				}, params.onComplete ?? require_app_call.OnApplicationComplete.NoOp);
			},
			deployUpdate: (params) => {
				return this.getABIParams(params, require_app_call.OnApplicationComplete.UpdateApplication);
			},
			deployDelete: (params) => {
				return this.getABIParams(params, require_app_call.OnApplicationComplete.DeleteApplication);
			},
			bare: {
				create: async (params) => {
				...
				deployUpdate: (params) => {
					return this.getBareParams(params, require_app_call.OnApplicationComplete.UpdateApplication);
				},
				deployDelete: (params) => {
	...
	*/
	...
	getABIParams(params, onComplete) {
		return {
			...
			sender: this.getSender(params.sender),
			signer: this.getSigner(params.sender, params.signer),
			method: require_abi_method.getABIMethod(params.method, this._appSpec),
